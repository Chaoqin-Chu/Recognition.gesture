function outline = ffsd(rFSDs,R,PR)
...
%R = iNoOfHarmonicsReconstruct
%PR = iNoOfPointsReconstruct
...
for iTime = 1:PR
    rSum = 0.0;
	for iHNo = iStartHarmonic:R
       	rSum = rSum + (rFSDs(1,iHNo) * cos(2*(iHNo-1)*pi*iTime / PR) + ...
            rFSDs(2,iHNo) * sin(2*(iHNo-1)*pi*iTime / PR));
	end % for iHNo = 1 : R
...
for iTime = 1:PR
    rSum = 0.0;
	for iHNo = iStartHarmonic:R
   	      rSum = rSum + (rFSDs(3,iHNo) * cos(2*(iHNo-1)*pi*iTime / PR) + ...
          	 rFSDs(4,iHNo) * sin(2*(iHNo-1)*pi*iTime / PR));
	end % for iHNo = 1 : R
   	ReconnedOutline(iTime,2) = rFSDs(3,1) + rSum;
end % for iTime = 1 : PR
outline = ReconnedOutline;
